...
# This allows PyTorch 2.14+ use nvidia-cudnn-cu13 of the venv in place of the
# cuDNN of the devel/cuda module/12.8, avoiding CUDNN_STATUS_SUBLIBRARY_LOADING_FAILED.
...
# --- Set up the venv under an exclusive lock: concurrent jobs share
# the venv, and pip is not safe in parallthe (corrupts numpy/scipy/onnx).
# Only this section is serialized; the subsequent pipeline runs in parallel.
...
# unconditional purging in each job is the main source of races.
...
PY
    ...
    # to avoid breaking previous runs that cached it
    if [[ -d "${SUPIR_DIR}" ]]; then
        export PYTHONPATH="${SUPIR_DIR}:${PYTHONPATH:-}"
    fi
...
echo "=============================================================================="
echo "Individual execution finished with exit code: ${EXIT_CODE}"
echo "=============================================================================="
